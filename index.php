...
if($mybb->user['uid'] == 0 || $mybb->usergroup['showforumteam'] != 1)
{
	error_no_permission();
}
...
$arrayY = array(
    '<a href="yetkili" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-home mr-2"></i>'.$lang->anasayfa.'</a>',
    '<a class="list-group-item list-group-item-action disabled py-2 px-3 text-muted fz-11">Oyuncu Yönetimi</a>',
    '<a href="yetkili?action=chat" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-comments mr-2"></i>'.$lang->chat_kayitlari.'</a>',
    '<a href="steamid" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-search mr-2"></i>'.$lang->steamidarama.'</a>',
    '<a href="yetkili?action=krediler" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-wallet mr-2"></i>'.$lang->krediler.'</a>',
    '<a href="rank" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-trophy mr-2"></i>'.$lang->rankadi.'</a>',
    '<a class="list-group-item list-group-item-action disabled py-2 px-3 text-muted fz-11">Sunucu Yönetimi</a>',
    '<a href="yetkili?action=adminuyarilari" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-user-injured mr-2"></i>'.$lang->yetkiliuyarilari.'</a>',
    '<a href="yetkili?action=adminlog" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-user-secret mr-2"></i>'.$lang->adminlog.'</a>',
    '<a href="yetkili?action=harita" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-map mr-2"></i>'.$lang->haritadongusu.'</a>',
    '<a href="yetkili?action=chatyazilari" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-comment-medical mr-2"></i>'.$lang->chatyazilari.'</a>',
    '<a href="yetkililer" class="list-group-item list-group-item-action py-2 px-3 fz-11"><i class="fas fa-users mr-2"></i>'.$lang->yetkililistesi.'</a>'
);

foreach ($arrayY as $value) {
    eval("\$yt_nav_row .= \"".$templates->get("yt_nav_row")."\";");
}

eval('$yt_nav  = "' . $templates->get('yt_nav', 1, 0) . '";');
...
if($mybb->input['action'] == "adminuyarilari") {

	add_breadcrumb($lang->adminlog, 'yonetici');

    if($mybb->usergroup['issupermod'] != 1)
    {
        eval("\$template .= \"".$templates->get("yt_error")."\";");output_page($template);
    }

    $query = $db->query("SELECT * FROM tf2turkiye_forum.mybb_users 
	WHERE usergroup = '9'
	OR usergroup = '30'
	OR usergroup = '31'
	OR usergroup = '32'
	...
	");
    while($row = $db->fetch_array($query)) {
        eval("\$yt_adminuyarilari_adminlistesirow .= \"".$templates->get("yt_adminuyarilari_adminlistesirow")."\";");
    }

    if($mybb->input['action'] == "adminuyarilari" AND $mybb->input['uid']) {
		

        $query = $db->query("SELECT * FROM tf2turkiye_forum.mybb_users WHERE uid = '" . $_GET['uid'] . "'");
        while($row = $db->fetch_array($query)) {
			eval("\$yt_adminuyarilarikayitlaricagir .= \"".$templates->get("yt_adminuyarilarikayitlaricagir")."\";");
        }
    }

    eval("\$template .= \"".$templates->get("yt_adminuyarilari")."\";");
...
if($mybb->get_input('action') == 'harita' && !$mybb->get_input('sid'))
{   
    add_breadcrumb($lang->haritadongusu, 'yonetici');
    
    
    
    $query = $db->query("SELECT sid, name, address FROM tf2turkiye_forum.mybb_sunucular ORDER BY sid ASC");
    while($row = $db->fetch_array($query)) {
        eval("\$yt_haritalar_row .= \"".$templates->get("yt_haritalar_row")."\";");
    }


    eval("\$template .= \"".$templates->get("yt_harita")."\";");
    output_page($template);
}

if($mybb->get_input('action') == 'harita' && $mybb->get_input('sid'))
{   
    switch ($mybb->get_input('sid')) {
        case '5':
            $ad = "Saxton Hale";
            $table = "mapcycle_sh";
            break;
    ...
    if($unknown == 1)
        error($lang->haritadongusuyok);
    
    $lang->haritadongusuduzenle = $lang->sprintf($lang->haritadongusuduzenle, $ad);
    add_breadcrumb($lang->haritadongusuduzenle, 'yonetici');
    
    
    
    $query = $db->query("SELECT * FROM tf2turkiye_cron." . $table . " ORDER by tf2turkiye_cron." . $table . ".id ASC");
    while($row = $db->fetch_array($query)) {
        if($mybb->usergroup['issupermod'] == 1)
        {
            eval("\$yt_harita_edit_row .= \"".$templates->get("yt_harita_edit_row")."\";");
        }
        else {
            eval("\$yt_harita_edit_row .= \"".$templates->get("yt_harita_edit_row_yetkisiz")."\";");
        }
    }
    ...
    if($mybb->usergroup['cancp'] == 1)
    {
        if(isset($_POST['guncelle'])) {
            $db->query("UPDATE tf2turkiye_cron." . $table . " SET id = '" . $_POST['id'] . "', mapname = '" . $_POST['mapname'] . "' WHERE id = '" . $_POST['orj_id'] ."'");
            $bildiri = bildiri($lang->kaydedildiyarin);
            redirect(NULL, $lang->kaydedildiyarin, NULL, 1);
        }

        if(isset($_POST['sil'])) {
            $db->query("DELETE FROM tf2turkiye_cron." . $table . " WHERE id = '" . $_POST['orj_id'] ."'");
            redirect(NULL, $lang->haritasilindi, NULL, 1);
        }

        if(isset($_POST['yeniharita'])) {
            $db->query("INSERT INTO tf2turkiye_cron." . $table . " (mapname) VALUES ('" . str_replace(".bsp","",$_POST['yeniharita']) ."')");
            redirect(NULL, $lang->haritaeklendi, NULL, 1);
        }

    $yeniharitaekle = '<form method="post" class="container pt-2 mt-2 border-top px-2" onSubmit="window.location.reload()">
                            <div class="row pt-2">
                                <div class="col-10">
                                <input type="text" name="yeniharita" value="'.$row['mapname'].'" class="form-control form-control-sm" placeholder="'.$lang->yeniharitaadi.'"></div>
                                <div class="col-auto ml-auto">
                                    <button type="submit" name="yeniekle" class="btn btn-success btn-sm"><i class="fas fa-plus mr-2"></i>'.$lang->yeniharitaekle.'</button>
                                </div>
                            </div>
                        </form>';
    ...
    eval("\$template .= \"".$templates->get("yt_harita_edit")."\";");
    output_page($template);
}

if($mybb->get_input('action') == 'adminlog')
{   
    add_breadcrumb($lang->adminlog, 'yonetici');
    ...
    $query = $db->query("SELECT * FROM tf2turkiye_cron.adminwatch");
    while($row = $db->fetch_array($query)) {
        eval("\$yt_adminlog_adminlistesirow .= \"".$templates->get("yt_adminlog_adminlistesirow")."\";");
    }

    if(isset($_GET['adminliste'])) {

    $query = $db->query("SELECT COUNT(id) as total FROM tf2turkiye_cron.adminwatch_logs WHERE steam = '".$_GET['adminliste']."'");
    ...
    {
    $perpage = 30; // I chose 10, but you can put what you like
    }
    ...
    $pagination = multipage($total, $perpage, $page, 'yetkili?action=adminlog&adminliste=' . $_GET['adminliste']);
    ...
    $offset = 0;
        ...
        $query = $db->query("SELECT * FROM tf2turkiye_cron.adminwatch INNER JOIN tf2turkiye_cron.adminwatch_logs ON tf2turkiye_cron.adminwatch.steam = tf2turkiye_cron.adminwatch_logs.steam WHERE tf2turkiye_cron.adminwatch_logs.steam = '".$_GET['adminliste']."' ORDER BY tf2turkiye_cron.adminwatch_logs.id DESC LIMIT $offset, $perpage");
        while($row = $db->fetch_array($query)) {
            $explode = explode("::", $row['hostname']);
                $server = explode("★", $explode[1]);

            $tarih = my_date('relative', $row['time']);
            eval("\$yt_adminlog_adminloglistesirow .= \"".$templates->get("yt_adminlog_adminloglistesirow")."\";");
        }
        eval("\$yt_adminlogkayitlaricagir .= \"".$templates->get("yt_adminlogkayitlaricagir")."\";");
    }

    eval("\$template .= \"".$templates->get("yt_adminlog")."\";");
    output_page($template);
}

if($mybb->get_input('action') == 'chatyazilari' && !$mybb->get_input('duzenle'))
{   
    add_breadcrumb($lang->chatyazilari, 'yonetici');
    ...
    $query = $db->query("SELECT * FROM tf2turkiye_cron.adsmysql");
    while($row = $db->fetch_array($query)) {
        $array = array(
            "{dimgray}[ {darkgray}SM {dimgray}] ",
            "{orange}",
            "{white}",
            "{gray}"
        );
        ...
        $row['text'] = str_replace($array,$replace,$row['text']);
        $row['flags'] = str_replace("none",$lang->herkes,$row['flags']);
        $row['gamesrvid'] = str_replace("All",$lang->hepsi,$row['gamesrvid']);
        $row['game'] = str_replace("All",$lang->hepsi,$row['game']);
        eval("\$yt_chatyazilarirow .= \"".$templates->get("yt_chatyazilarirow")."\";");
    }

    eval("\$template .= \"".$templates->get("yt_chatyazilari")."\";");
    output_page($template);
}
if($mybb->get_input('action') == 'chatyazilari' && $mybb->get_input('duzenle'))
{   
    add_breadcrumb($lang->otochatyazilari_duzenle, 'yonetici');

    $query = $db->query("SELECT * FROM tf2turkiye_cron.adsmysql WHERE id = " . $mybb->get_input('duzenle'));
    $row = $db->fetch_array($query);

    $query = $db->query("SELECT tf2turkiye_forum.".TABLE_PREFIX."sunucular.id, tf2turkiye_forum.".TABLE_PREFIX."sunucular.name, tf2turkiye_forum.".TABLE_PREFIX."sunucular.hostname, tf2turkiye_ceza.ceza_servers.sid, tf2turkiye_ceza.ceza_servers.ip FROM tf2turkiye_forum.".TABLE_PREFIX."sunucular INNER JOIN tf2turkiye_ceza.ceza_servers ON tf2turkiye_forum.".TABLE_PREFIX."sunucular.address = tf2turkiye_ceza.ceza_servers.ip ORDER BY tf2turkiye_ceza.ceza_servers.sid ASC");
    while($sunucular = $db->fetch_array($query)) {
        $explode = explode("::", $sunucular['hostname']);
            $server = explode("★", $explode[1]);
        $secili = explode(',', $row['gamesrvid']);
        foreach ($secili as $key => $value) {
            if($value == $sunucular['sid'])
                $checked = " checked";
            else
                $checked = "";
        }
                eval("\$yt_chatyazilarirow_edit_sunuculisterow .= \"".$templates->get("yt_chatyazilarirow_edit_sunuculisterow")."\";");
    }

    if(isset($_POST['kaydet'])) {

        if($_POST['gamesrvid'] == 'Hepsi') {
            $secilisunucular = "All";
        }
        elseif($_POST['gamesrvid'] == NULL) {
            $secilisunucular = 0;
            $secilisunucular = array();
            $query = $db->query("SELECT * FROM tf2turkiye_ceza.ceza_servers");
            for ($i=1; $i <= $db->num_rows($query); $i++) { 
                $deger = 'gamesrvid_' . $i;
                if($_POST[$deger] != NULL) {
                    array_push($secilisunucular, $_POST[$deger]);
                }
            }
            $secilisunucular = implode(",", $secilisunucular);
        }
        else {
            $secilisunucular = "All";
        }
        if($_POST['game'] == "Team Fortress 2") {
            $_POST['game'] = "tf";
        }
        else {
            $_POST['game'] = "All";
        }
        $db->query("UPDATE tf2turkiye_cron.adsmysql SET id = '" . $_POST['id'] . "', type = '" . $_POST['type'] . "', text = '" . $_POST['text'] . "', flags = '" . $_POST['flags'] . "', game = '" . $_POST['game'] . "', gamesrvid = '" . $secilisunucular . "',  name = '" . $mybb->user['username'] . "' WHERE id = '" . $_POST['orj_id'] ."'");
        $bildiri = bildiri($lang->kaydedildiyarin);
        redirect(NULL, $lang->kaydedildiyarin, NULL, 1);
    }

    if(isset($_POST['sil'])) {
        $db->query("DELETE FROM tf2turkiye_cron.adsmysql WHERE id = '" . $_POST['orj_id'] ."'");
        redirect(NULL, $lang->haritasilindi, NULL, 1);
    }
    ...
    eval("\$template .= \"".$templates->get("yt_chatyazilarirow_edit")."\";");
    output_page($template);
}


if($mybb->get_input('action') == 'krediler' && !$mybb->get_input('id') && !$mybb->get_input('duzenle'))
{   
    add_breadcrumb($lang->krediler, 'yonetici');
    ...
    $query = $db->query("SELECT COUNT(id) as total FROM tf2turkiye_cron.store_players");
    $total = $db->fetch_field($query, "total");
    ...
    $pagination = multipage($total, $perpage, $page, 'yetkili?action=krediler');
    ...
    switch ($_POST['yontem']) {
        case 'steamid':
        $query = $db->query("SELECT * FROM tf2turkiye_cron.store_players WHERE authid = '". str_replace("STEAM_0:","",$_POST['arama']) ."' ORDER BY tf2turkiye_cron.store_players.id " . $_GET['siralama'] . " LIMIT $offset, $perpage");
            break;
        case 'nick':
        $query = $db->query("SELECT * FROM tf2turkiye_cron.store_players WHERE name = '". $_POST['arama'] ."' ORDER BY tf2turkiye_cron.store_players.id " . $_GET['siralama'] . " LIMIT $offset, $perpage");
            break;
        
        default:
        $query = $db->query("SELECT * FROM tf2turkiye_cron.store_players ORDER BY tf2turkiye_cron.store_players.id " . $_GET['siralama'] . " LIMIT $offset, $perpage");
            break;
    }
    if($_GET['siralama']) {
        $query = $db->query("SELECT * FROM tf2turkiye_cron.store_players ORDER BY tf2turkiye_cron.store_players.credits " . $_GET['siralama'] . " LIMIT $offset, $perpage");
    }

        while($row = $db->fetch_array($query)) {
            $row['name'] = '<a href="https://steamcommunity.com/profiles/' . steamid64('STEAM_0:' . $row['authid']) . '" target="_blank">' . $row['name'] . '</a>';
            $row['date_of_join'] = my_date('relative', $row['date_of_join']);
            $row['date_of_last_join'] = my_date('relative', $row['date_of_last_join']);

            if($mybb->usergroup['cancp'] == 1)
            {
                $dugme = '<a href="?action=krediler&duzenle='.$row['id'].'" class="btn btn-sm bg-transparent border-0 text-primary p-0"><i class="fas fa-pen-alt mr-2"></i>'.$lang->duzenle.'</a>';
            }

            eval("\$yt_krediler_row .= \"".$templates->get("yt_krediler_row")."\";");
        }


    eval("\$template .= \"".$templates->get("yt_krediler")."\";");
    output_page($template);
} 

if($mybb->get_input('action') == 'krediler' && $mybb->get_input('duzenle'))
{   
    add_breadcrumb($lang->krediler_duzenle, 'yonetici');
    ...
    $query = $db->query("SELECT * FROM tf2turkiye_cron.store_players WHERE id = '". $_GET['duzenle'] ."'");
    
    while($row = $db->fetch_array($query)) {
        $id = $row['id'];
        $name = $row['name'];
        $authid = 'STEAM_0:' . $row['authid'];
        $credits = $row['credits'];
    }
    ...
    {
        if(isset($_POST['kaydet'])) {
            $db->query("UPDATE tf2turkiye_cron.store_players SET credits = '" . $_POST['credits'] . "' WHERE id = '" . $_GET['duzenle'] ."'");
            $bildiri = bildiri($lang->kaydedildi);
            redirect(NULL, $lang->kaydedildi, NULL, 1);
        }
    }


    eval("\$template .= \"".$templates->get("yt_krediler_edit")."\";");
    output_page($template);
}


if($mybb->get_input('action') == 'chat' && !$mybb->get_input('sid')) {
    add_breadcrumb($lang->chatkayitlari, 'yonetici');
    
    $query = $db->query("SELECT sid, name, address FROM tf2turkiye_forum.mybb_sunucular ORDER BY sid ASC");
    while($row = $db->fetch_array($query)) {
        eval("\$yt_chat_sunucular .= \"".$templates->get("yt_chat_sunucular")."\";");
    }

    eval("\$template .= \"".$templates->get("yt_chat")."\";");
    output_page($template);
}
...
if($mybb->get_input('action') == 'chat' && $mybb->get_input('sid')) {
    ...
    // $ip='185.93.70.10' . $mybb->get_input('sid');
    // $mysqli = mysqli_connect($ip,"root","1Q2w3a4s++kerem","veritabani");

    switch ($_GET['yontem']) {
        case 'nick':
        $query = $db->query("SELECT seqid FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND name LIKE '%".$mybb->get_input('arama')."%'");
            break;
        case 'steamid':
        $query = $db->query("SELECT seqid FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND steamid = '".$mybb->get_input('arama')."'");
            break;
        case 'mesaj':
        $query = $db->query("SELECT seqid FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND name LIKE '%".$mybb->get_input('arama')."%'");
            break;
        ...
        default:
		$query = $db->query("SELECT seqid FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0");
            break;
    }
    ...
    if($mybb->get_input('siralama') != 'ASC')
    $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 ORDER BY seqid DESC LIMIT $offset, $perpage");
    else
    $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 ORDER BY seqid ASC LIMIT $offset, $perpage");

    switch ($_GET['yontem']) {
        case 'nick':
        $pagination = multipage($total, $perpage, $page, 'yetkili?action=chat&sid=' . $mybb->get_input('sid') . '&yontem=' . $mybb->get_input('yontem') . '&arama=' . $mybb->get_input('arama') );
        if($mybb->get_input('siralama') != 'ASC')
            $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND name LIKE '%".$mybb->get_input('arama')."%' ORDER BY seqid DESC LIMIT $offset, $perpage");
        else
            $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND name LIKE '%".$mybb->get_input('arama')."%' ORDER BY seqid ASC LIMIT $offset, $perpage");
            break;
        case 'steamid':
        $pagination = multipage($total, $perpage, $page, 'yetkili?action=chat&sid=' . $mybb->get_input('sid') . '&yontem=' . $mybb->get_input('yontem') . '&arama=' . $mybb->get_input('arama') );
        $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND steamid = '".$mybb->get_input('arama')."' ORDER BY seqid DESC LIMIT $offset, $perpage");
            break;
        case 'mesaj':
        $pagination = multipage($total, $perpage, $page, 'yetkili?action=chat&sid=' . $mybb->get_input('sid') . '&yontem=' . $mybb->get_input('yontem') . '&arama=' . $mybb->get_input('arama') );
        $query = $db->query("SELECT * FROM tf2turkiye_cron.chatlogs_".$mybb->get_input('sid')." WHERE type > 0 AND text LIKE '%".$mybb->get_input('arama')."%' ORDER BY seqid DESC LIMIT $offset, $perpage");
            break;
        
        default:
        $pagination = multipage($total, $perpage, $page, 'yetkili?action=chat&sid=' . $mybb->get_input('sid'));
    ...
    while($row = $query->fetch_array()) {
        ...
        $row['name'] = '<span class="text-' . $renk . '">' . $row['name'] . '</span>';
        $row['date'] = date("d M H:i", strtotime($row['date']));
        eval("\$yt_chat_row .= \"".$templates->get("yt_chat_row")."\";");
    }

    eval("\$template .= \"".$templates->get("yt_chat_liste")."\";");
    output_page($template);
}
