...
if($mybb->user['usergroup'] != 1 || $mybb->user['usergroup'] != 2 || $mybb->user['usergroup'] != 5 || $mybb->user['usergroup'] != 6 || $mybb->user['usergroup'] != 7 || $mybb->user['usergroup'] != 8) {
	$authid = steamid32(str_replace("@steamcommunity.com","",$mybb->user['email']));
	$query = $db->query("SELECT authid, email AS sure FROM tf2turkiye_ceza.ceza_admins WHERE authid = '".$authid."'");
	$sure = $db->fetch_field($query, "sure");
	if ($sure != 0) {
		$kalan = round(($sure - time()) / 86400);
		$kalansure = '<small class="dropdown-item disabled px-3 py-2">
		<div class="row vertical-align">
			<div class="col'.($kalan < 7 ? " text-danger font-weight-bold" : "").'">
				 Yetkinin '.$kalan. $lang->gunkaldi . '
			</div>
			<div class="col-auto ml-auto">
					<a href="' . $mybb->settings['bburl'] . '/'. ( $mybb->user['usergroup'] == 10 ? "premium" : "adminlik" ) .'" class="text-black-50"><i class="fas fa-plus-circle text-success" data-toggle="tooltip" data-placement="bottom" data-original-title="Süreyi uzatmak için tıkla"></i></a>
			</div>
		</div>
	</small>';
	}
...
$ad = $ua['name'];
$tara = "Chrome";
$konum = strstr($samanlik, $igne);
if (strstr($ad, "Firefox") || strstr($ad, "Safari")) {
$lang->browserkontrol = $lang->sprintf($lang->browserkontrol, $ua['name']);
eval('$tarayici = "'.$templates->get('sayfa_tarayici').'";');
} else {
...
if($mybb->user['uid'] != 0)
{
	...
	{
		$query = $db->simple_select('threads', 'COUNT(prefix) as toplambekleyensikayet', "prefix='0' AND (fid='26' AND sticky='0' AND closed <> 1 )");
		$total_sikayet = $db->fetch_field($query, 'toplambekleyensikayet');
		if($total_sikayet != 0) {
			$lang->bekleyensikayetler = $lang->sprintf($lang->bekleyensikayetler, $total_sikayet);
			$bekleyensikayetler = '<span class="position-absolute top-0 right-0 bg-success text-white fz-7 mr-2 rounded p-1 text-center">' . $total_sikayet . '</span>';
			eval('$header_sikayetler = "'.$templates->get('header_sikayetler').'";');
		}

		$query = $db->simple_select('posts', 'COUNT(visible) as toplamonay', "visible='0'");
		$total_bekleyenonay = $db->fetch_field($query, 'toplamonay');
		if($total_bekleyenonay != 0) {
			$lang->bekleyenbekleyenonay = $lang->sprintf($lang->bekleyenbekleyenonay, $total_bekleyenonay);
			$bekleyenbekleyenonay = '<span class="position-absolute top-0 right-0 bg-primary text-white fz-7 mr-2 rounded p-1 text-center">' . $total_bekleyenonay . '</span>';
			eval('$header_bekleyenonay = "'.$templates->get('header_bekleyenonay').'";');
		}
	...
	{
		$query = $db->simple_select('reportedcontent', 'COUNT(reportstatus) as toplambekleyenrapor', "reportstatus='0'");
		$total_reports = $db->fetch_field($query, 'toplambekleyenrapor');
		if($total_reports != 0) {
			$lang->bekleyenraporsayisi = $lang->sprintf($lang->bekleyenraporsayisi, $total_reports);
			$bekleyenrapor = '<span class="position-absolute top-0 right-0 bg-info text-white fz-7 mr-2 rounded p-1 text-center">' . $total_reports . '</span>';
			eval('$header_raporlar = "'.$templates->get('header_raporlar').'";');
		}
	}
	...
	if($mybb->user['usergroup'] == 2) {
		$premium = '<a class="dropdown-item px-3 py-2 border-bottom text-purple" href="' . $mybb->settings['bburl'] . '/premium">
		<div class="row">
			<div class="col-1">
				<i class="fas fa-star mr-2 text-purple d-inline-block"></i>
			</div>
			<div class="col-10">
				' . $lang->header_premiumuyelik . '
			</div>
		</div>
	</a>';
	}
...
if(isset($mybb->user['pmnotice']) && $mybb->user['pmnotice'] == 2 && $mybb->user['pms_unread'] > 0 && $mybb->settings['enablepms'] != 0 && $mybb->usergroup['canusepms'] != 0 && $mybb->usergroup['canview'] != 0 && ($current_page != "private.php" || $mybb->get_input('action') != "read"))
{
	...
	$query = $db->query("
	SELECT pm.subject, pm.dateline, pm.pmid, pm.fromid, fu.username AS fromusername, fu.uid AS fromuid
	...
	while($pm = $db->fetch_array($query)) {
		...
		$lastpost_date = my_date('relative', $pm['dateline']);
		...
		else
		{
			...
			$user_text = mb_strimwidth(htmlspecialchars_uni($pm['fromusername']), 0, 25, '...');
		...
		{
			$avatar2=htmlspecialchars_uni($pm['fromid']);
			$query2 = $db->simple_select("users", "avatar", "uid=$avatar2");
			$avatar = $db->fetch_field($query2, "avatar");
			$avatar = str_replace("_full","_medium",$avatar);
			$privatemessage_text2 = $lang->sprintf($lang->newpm_notice_multiple2, $mybb->user['pms_unread']);
			$privatemessage_text = $lang->sprintf($lang->newpm_notice_multiple, $mybb->user['pms_unread'], $user_text, $mybb->settings['bburl'], $pm['pmid'], mb_strimwidth(htmlspecialchars_uni($pm['subject']), 0, 35, '...'), $lastpost_date, $avatar);
		}

		eval("\$pm_notice .= \"".$templates->get("global_pm_alert2")."\";");
	}
}
...
if($mybb->user['uid'] && $mybb->user['fid7'] == NULL) {
	eval('$header_discord = "'.$templates->get('header_discord').'";');
}


if($mybb->user['uid'] && $mybb->user['unreadAlerts'] > 0) {
	$lang->bildirim = '(' . $mybb->user['unreadAlerts'] . ') ' ;
}
$query = $db->simple_select("threads", "tid", "fid='30'", array(
    "order_by" => 'tid',
    "order_dir" => 'DESC',
    "limit" => 1
));
$soncekilis = $db->fetch_array($query);
$soncekilis = get_thread_link($soncekilis['tid']);
...
eval('$header_minimal = "'.$templates->get('header_minimal').'";');
...
$navbits = array();
$navbits[0]['name'] = '<i class="fas fa-comments mr-2"></i>' . $lang->forumjump_home;
$navbits[0]['url'] = 'index.php';
